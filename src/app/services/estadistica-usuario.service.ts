...
export class EstadisticaService {
    private readonly BASE = '/api/usuarios/estadisticas';

    constructor(private http: HttpClient) { }

    getUsuariosPorTipo(page = 0, size = 10, ordenarPor = 'tipo', ascendente = true)
        ...
        return this.http.get<Page<EstadisticaUsuario>>(
            `${this.BASE}/tipo`, { params }
        );
    }

    exportUsuariosPorTipoCsv(): Observable<Blob> {
        return this.http.get(
            `${this.BASE}/tipo/export/csv`, { responseType: 'blob' }
        );
    }
    ...
    getComprasClientes(
        ...
        return this.http.get<Page<EstadisticaClienteCompras>>(
            `${this.BASE}/compras-clientes`, { params }
        );
    }

    exportComprasClientesCsv(fechaInicio?: string, fechaFin?: string): Observable<Blob> {
        let params = new HttpParams();
        if (fechaInicio) params = params.set('fechaInicio', fechaInicio);
        if (fechaFin) params = params.set('fechaFin', fechaFin);
        return this.http.get(
    ...
    getLogueosUsuarios(
        ...
        return this.http.get<Page<EstadisticaLogueos>>(
            `${this.BASE}/logueos`, { params }
        );
    }

    exportLogueosCsv(fechaInicio?: string, fechaFin?: string): Observable<Blob> {
        let params = new HttpParams();
        if (fechaInicio) params = params.set('fechaInicio', fechaInicio);
        if (fechaFin) params = params.set('fechaFin', fechaFin);
        return this.http.get(
